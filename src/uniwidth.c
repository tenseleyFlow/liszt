/* Unicode width tables: gnulib uniwidth port (coreutils 9.11 vintage,
   Unicode 17.0.0 tables). Verbatim table data from lib/uniwidth/
   width0.h and width2.h plus the three-level bitmap lookup from
   lib/unictype/bitmap.h; uc_width logic from lib/uniwidth/width.c with
   the ancient-CJK-encoding branch dropped - liszt consults this table
   only in UTF-8 locales, exactly where gnulib's replacement wcwidth
   does (REPLACE_WCWIDTH platforms). LGPL-2.1-or-later, FSF. */
...
/* unictype/bitmap.h three-level lookup. The header_* constants are
   defined by the width2.h blob above. */
static int
bitmap_lookup(const void *table, ucs4_t uc)
{
    ...
    if (index1 < (unsigned int)((const int *)table)[0]) {
        ...
        if (lookup1 >= 0) {
            size_t index2 = (uc >> header_2) & header_3;
            int lookup2 = ((const short *)table)[(size_t)lookup1 + index2];
            if (lookup2 >= 0) {
                size_t index3 = (uc >> 5) & header_4;
                unsigned int lookup3 =
                    ((const unsigned int *)table)[(size_t)lookup2 + index3];
...
int
liszt_uc_width(uint32_t uc)
{
    /* Test for non-spacing or control character. */
    if ((uc >> 9) < sizeof nonspacing_table_ind / sizeof nonspacing_table_ind[0]) {
        int ind = nonspacing_table_ind[uc >> 9];
        if (ind >= 0)
            if ((nonspacing_table_data[64 * (size_t)ind + ((uc >> 3) & 63)]
                 >> (uc & 7)) & 1) {
                if (uc > 0 && uc < 0xa0)
                    return -1;
                else
